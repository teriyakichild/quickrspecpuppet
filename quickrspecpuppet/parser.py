...
class ManifestParser(object):
    ...
    def parse_modulefile(self):
        ...
        if os.path.exists(modulefile_path):
            logger.debug('DEBUG: modulefile {0} exists'.format(modulefile_path))
...
class PuppetDependency(object):
    def __init__(self, name):
        ...
        try:
            ...
            raise Exception('Invalid format for puppet module name ({0}), should be "owner/repo_name"'.format(name))
